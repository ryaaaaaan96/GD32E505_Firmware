...
static void statusTask(void *argument)
{
    ...
    for (;;) {
        if (aDevLedToggle(&s_status_led) != A_STATUS_OK) {
            ...
            for (;;) {
                aOSDelayMs(SYSTEM_STATUS_BLINK_PERIOD_MS);
            }
        }
        aOSDelayMs(SYSTEM_STATUS_BLINK_PERIOD_MS);
    }
...
static aStatus_t statusInit(void)
{
    ...
    config.pin = SYSTEM_STATUS_LED_PIN;
    config.active_level = SYSTEM_STATUS_LED_ACTIVE_LEVEL;
...
static uint8_t s_shell_rx_buffer[SYSTEM_SHELL_RX_BUFFER_SIZE];
static uint8_t s_shell_tx_buffer[SYSTEM_SHELL_TX_BUFFER_SIZE];

static int16_t shellWrite(char *buffer, uint16_t size)
{
    const aSSize_t result = aDevUsartWrite(
        &s_shell_usart, buffer, size, SYSTEM_SHELL_IO_TIMEOUT);
...
static aStatus_t shellUsartInit(void)
{
    ...
    config.drv_config.id = SYSTEM_SHELL_USART;
    config.drv_config.tx_pin = SYSTEM_SHELL_TX_PIN;
    config.drv_config.rx_pin = SYSTEM_SHELL_RX_PIN;
    config.drv_config.baud_rate = SYSTEM_SHELL_BAUD_RATE;
    config.mode = SYSTEM_SHELL_USART_MODE;
    config.interrupt_priority = SYSTEM_SHELL_IRQ_PRIORITY;
