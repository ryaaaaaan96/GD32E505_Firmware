 ...
 *
 * aDevUsart 在 aDrv 的非阻塞硬件操作之上提供轮询、中断缓冲、DMA 和统一超时
 * 语义。阻塞等待使用 aOS 的单调时基和 yield，因此本文件中的 read/write/等待
 * 接口只能在任务或线程上下文调用，不能在 ISR 中调用。
 *
 ...
 * @brief USART 实例的运行方式。
 *
 * 模式只决定当前设备实例如何收发数据；对应能力是否编译进固件由 aDrv 配置
 * 决定。选择了当前 port 不支持或未编译的能力时，aDevUsartInit() 返回
 * A_STATUS_UNSUPPORTED。
 */
typedef enum {
    /** 发送轮询 TBE，接收轮询 RBNE；不使用软件缓冲区、中断或 DMA。 */
    ADEV_USART_MODE_POLLING,

    /** 发送使用 DMA，接收轮询 RBNE；配置中的 RX/TX 缓冲区均不使用。 */
    ADEV_USART_MODE_DMA_TX,

    /**
     * 发送使用 TX 环形缓冲区和 TBE 中断；接收使用 RX 环形缓冲区和 RBNE
     * 中断；同时记录 USART IDLE 事件。需要 rx_buffer 和 tx_buffer。
     */
    ADEV_USART_MODE_INTERRUPT_IDLE,

    /**
     * 发送使用 TX 环形缓冲区和 TBE 中断；接收 DMA 以循环模式直接写入 RX
     * 环形缓冲区。IDLE 只提交 DMA 写入位置，不停止 DMA，也不复制数据。
     * 需要 rx_buffer 和 tx_buffer，且 aDrv 必须同时提供 USART 中断与循环
     * DMA 接收能力。
     */
 ...
 * 模式和模式所需缓冲区。配置结构本身只在初始化期间读取，但其中指向的缓冲区
 * 会由设备长期使用。
 */
typedef struct {
    ...
    /** 当前实例的运行模式，默认 ADEV_USART_MODE_POLLING。 */
    aDevUsartMode_t mode;

    /**
     * USART 中断优先级，仅中断相关模式使用；取值必须满足当前 aDrv port 的
     * 优先级范围以及所使用 RTOS 的 ISR 调用约束。
     */
    uint8_t interrupt_priority;

    /**
     * 应用可读取的 RX 环形缓冲区。中断接收模式和 DMA RX + IDLE 模式必需。
     * 在 DMA RX 模式中，这也是 DMA 直接写入的循环目标缓冲区。
     */
    uint8_t *rx_buffer;
 ...
 *
 * 默认使用轮询模式、中断优先级 5，所有设备缓冲区为空；aDrv 子配置由
 * aDrvUsartConfigStructInit() 初始化。
 *
 ...
 *
 * DMA 模式会先终止相应 DMA 操作，再关闭底层 USART。调用期间不得有其他任务
 * 正在访问该 handle。成功后可以重新调用 aDevUsartHandleStructInit() 并复用它。
 *
 ...
 *
 * 仅 IDLE 相关模式具有业务意义。计数用于观察事件变化，不代表 RX 环形缓冲区
 * 当前字节数，并允许自然回绕。handle 为 NULL 时返回 0。
 */
